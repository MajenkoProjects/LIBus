...
void LIBusSpinner::absolute(uint8_t led, uint8_t value) {
    absolute_t data;
    data.led = led;
    data.value = value;
...
void LIBusSpinner::fade(uint8_t led, uint8_t value, uint16_t ms) {
    fade_t data;
    data.led = led;
    data.value = value;
    data.ms = ms;
...
void LIBusSpinner::blink(uint8_t led, uint16_t marktime, uint16_t spacetime, uint8_t markvalue, uint8_t spacevalue) {
    blink_t data;
    data.led = led;
...
void LIBusSpinner::strobe(uint8_t led, uint16_t marktime, uint16_t spacetime, uint16_t breaktime, uint8_t blinks, uint8_t markvalue, uint8_t spacevalue, uint8_t breakvalue) {
    strobe_t data;
    data.led = led;
    data.marktime = marktime;
    data.spacetime = spacetime;
...
void LIBusSpinner::sync(uint8_t led, uint8_t reference) {
    sync_t data;
    data.led = led;
...
void LIBusSpinner::flicker(uint8_t led, uint8_t minvalue, uint8_t maxvalue, uint16_t ms) {
    flicker_t data;
    data.led = led;
...
void LIBusSpinner::setMode(uint8_t mode) {
    _libus->send(_address, 0x20, (uint8_t *)&mode, 1);
}
